...
class Tambah_Retur_Pembelian extends CI_Controller {

	public function __construct() {
		parent::__construct();
		$this->load->model('Mdl_tambahreturpembelian');
		$this->auth->restrict();
	...
	function pilihitemretur(){
       // $this->mdl_home->getsqurity();
        $data['view_file']    = "Tampil_Data_Transaksi/view_tampil_data_transaksi";
        $this->load->view('admin_view',$data);
    }

     public function ajax_list() {
		$list = $this->Mdl_tambahreturpembelian->get_datatables();
		$data = array();
		$no = $_REQUEST['start'];
		...
		$output = array(
						"draw" => $_REQUEST['draw'],
						"recordsTotal" => $this->Mdl_tambahreturpembelian->count_all(),
						"recordsFiltered" => $this->Mdl_tambahreturpembelian->count_filtered(),
						"data" => $data,
				);

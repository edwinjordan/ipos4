...
class Tambah_Daftar_Penjualan extends CI_Controller {

	public function __construct() {
		parent::__construct();
		$this->load->model('Mdl_tambahdaftarpenjualan');
		$this->auth->restrict();
	...
	function index(){
       // $this->mdl_home->getsqurity();
        $data['view_file']    = "daftar_penjualan/view_tambah_daftar_penjualan";
...
function ceksupplier(){
     ...
     public function ajax_list() {
		$list = $this->Mdl_tambahdaftarpenjualan->get_datatables();
		$data = array();
		$no = $_REQUEST['start'];
		...
		$output = array(
						"draw" => $_REQUEST['draw'],
						"recordsTotal" => $this->Mdl_tambahdaftarpenjualan->count_all(),
						"recordsFiltered" => $this->Mdl_tambahdaftarpenjualan->count_filtered(),
						"data" => $data,
				);
		echo json_encode($output);
	}
	public function ajax_list1() {
		$list = $this->Mdl_tambahdaftarpenjualan->get_datatables2();
		$data = array();
		$no = $_REQUEST['start'];
		foreach ($list as $album) {
			if($album->album_gambar==''){ $cover = 'no_image.jpg'; }else{ $cover = $album->album_gambar; }
		...
		$output = array(
						"draw" => $_REQUEST['draw'],
						"recordsTotal" => $this->Mdl_tambahdaftarpenjualan->count_all2(),
						"recordsFiltered" => $this->Mdl_tambahdaftarpenjualan->count_filtered2(),
						"data" => $data,
				);
		echo json_encode($output);
	}

...
class Tambah_Pesanan_Pembelian extends CI_Controller {

	public function __construct() {
		parent::__construct();
		$this->load->model('Mdl_tambahpesananpembelian');
		$this->auth->restrict();
	...
	function index(){
       // $this->mdl_home->getsqurity();
        $data['view_file']    = "pesanan_pembelian/view_tambah_pesanan_pembelian";
...
function ceksupplier(){
     ...
     public function ajax_list() {
		$list = $this->Mdl_tambahpesananpembelian->get_datatables();
		$data = array();
		$no = $_REQUEST['start'];
		...
		$output = array(
						"draw" => $_REQUEST['draw'],
						"recordsTotal" => $this->Mdl_tambahpesananpembelian->count_all(),
						"recordsFiltered" => $this->Mdl_tambahpesananpembelian->count_filtered(),
						"data" => $data,
				);

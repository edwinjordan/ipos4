...
class Daftar_Penjualan extends CI_Controller {
	...
	public function __construct() {
		parent::__construct();
		$this->load->model('Mdl_daftarpenjualan');
		$this->auth->restrict();
	...
	function index(){
       // $this->mdl_home->getsqurity();
        $data['view_file']    = "daftar_penjualan/view_daftar_penjualan";
        $this->load->view('admin_view',$data);
    }

    function tambah(){
       // $this->mdl_home->getsqurity();
        $data['view_file']    = "tambah_daftar_penjualan/view_tambah_daftar_penjualan";
        $this->load->view('admin_view',$data);
    }

     public function ajax_list() {
		$list = $this->Mdl_daftarpenjualan->get_datatables();
		$data = array();
		$no = $_REQUEST['start'];
		...
		$output = array(
						"draw" => $_REQUEST['draw'],
						"recordsTotal" => $this->Mdl_daftarpenjualan->count_all(),
						"recordsFiltered" => $this->Mdl_daftarpenjualan->count_filtered(),
						"data" => $data,
				);

...
class History_Harga_Jual extends CI_Controller {

	public function __construct() {
		parent::__construct();
		$this->load->model('Mdl_historyhargajual');
		$this->auth->restrict();
	...
	function index(){
       // $this->mdl_home->getsqurity();
        $data['view_file']    = "history_harga_jual/view_history_harga_jual";
    ...
    function cekpelanggan(){
     ...
     public function ajax_list() {
		$list = $this->Mdl_historyhargajual->get_datatables();
		$data = array();
		$no = $_REQUEST['start'];
		...
		$output = array(
						"draw" => $_REQUEST['draw'],
						"recordsTotal" => $this->Mdl_historyhargajual->count_all(),
						"recordsFiltered" => $this->Mdl_historyhargajual->count_filtered(),
						"data" => $data,
				);

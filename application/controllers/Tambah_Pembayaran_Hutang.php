...
class Tambah_Pembayaran_Hutang extends CI_Controller {

	public function __construct() {
		parent::__construct();
		$this->load->model('Mdl_tambahpembayaranhutang');
		$this->auth->restrict();
	...
	function index(){
       // $this->mdl_home->getsqurity();
        $data['view_file']    = "daftar_pembayaran/view_tambah_pembayaran_hutang";
        $this->load->view('admin_view',$data);
    }

     public function ajax_list() {
		$list = $this->Mdl_tambahpembayaranhutang->get_datatables();
		$data = array();
		$no = $_REQUEST['start'];
		...
		$output = array(
						"draw" => $_REQUEST['draw'],
						"recordsTotal" => $this->Mdl_tambahpembayaranhutang->count_all(),
						"recordsFiltered" => $this->Mdl_tambahpembayaranhutang->count_filtered(),
						"data" => $data,
				);

...
class Penjualan_Kasir extends CI_Controller {
	...
	public function __construct() {
		parent::__construct();
		$this->load->model('Mdl_penjualankasir');
		$this->auth->restrict();
	...
	function index(){
       // $this->mdl_home->getsqurity();
        $data['view_file']    = "penjualan_kasir/view_penjualan_kasir";
        $this->load->view('admin_view',$data);
    }

    function tambah(){
       // $this->mdl_home->getsqurity();
        $data['view_file']    = "tambah_penjualan_kasir/view_tambah_penjualan_kasir";
        $this->load->view('admin_view',$data);
    }

     public function ajax_list() {
		$list = $this->Mdl_penjualankasir->get_datatables();
		$data = array();
		$no = $_REQUEST['start'];
		...
		$output = array(
						"draw" => $_REQUEST['draw'],
						"recordsTotal" => $this->Mdl_penjualankasir->count_all(),
						"recordsFiltered" => $this->Mdl_penjualankasir->count_filtered(),
						"data" => $data,
				);

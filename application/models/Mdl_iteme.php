...
class Mdl_iteme extends CI_Model {
  ...
  public function get_datatablesid($id, $id2, $id3, $id4) {
    ...
    $this->db->where('a.item_qty <=','a.item_stok_min');
    $this->db->order_by($id4,'asc');
	...
	function count_filteredid($id, $id2, $id3, $id4) {
		$this->_get_datatables_queryid();
		$this->db->where('a.kode_item',$id);
    $this->db->where('a.kode_item_jenis',$id2);
    $this->db->where('a.kode_gudang',$id3);

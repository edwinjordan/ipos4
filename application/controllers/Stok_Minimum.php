...
class Stok_Minimum extends CI_Controller {
	...
	public function __construct() {
		parent::__construct();
		$this->load->model('Mdl_stokminimum');
		$this->auth->restrict();
    ...
    public function ajax_list() {
		$list = $this->Mdl_stokminimum->get_datatables();
		$data = array();
		$no = $_REQUEST['start'];
		...
		$output = array(
						"draw" => $_REQUEST['draw'],
						"recordsTotal" => $this->Mdl_stokminimum->count_all(),
						"recordsFiltered" => $this->Mdl_stokminimum->count_filtered(),
						"data" => $data,
				);

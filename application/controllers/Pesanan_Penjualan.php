...
class Pesanan_Penjualan extends CI_Controller {
	...
	public function __construct() {
		parent::__construct();
		$this->load->model('Mdl_pesananpenjualan');
		$this->auth->restrict();
	...
	function index(){
       // $this->mdl_home->getsqurity();
        $data['view_file']    = "pesanan_penjualan/view_pesanan_penjualan";
        $this->load->view('admin_view',$data);
    }

    function tambah(){
       // $this->mdl_home->getsqurity();
        $data['view_file']    = "tambah_pesanan_penjualan/view_tambah_pesanan_penjualan";
        $this->load->view('admin_view',$data);
    }
    ...
    function cekpelanggan(){
       // $this->mdl_home->getsqurity();
        $data['view_file']    = "tampil_pelanggan/view_tampil_pelanggan";
        $this->load->view('admin_view',$data);
    }

     public function ajax_list() {
		$list = $this->Mdl_pesananpenjualan->get_datatables();
		$data = array();
		$no = $_REQUEST['start'];
		...
		$output = array(
						"draw" => $_REQUEST['draw'],
						"recordsTotal" => $this->Mdl_pesananpenjualan->count_all(),
						"recordsFiltered" => $this->Mdl_pesananpenjualan->count_filtered(),
						"data" => $data,
				);
